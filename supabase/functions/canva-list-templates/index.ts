...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return json({ error: `Canva templates fetch failed: ${t.slice(0, 500)}`, status: r.status }, 500);
    }
    const data = await r.json();
    // shape: { items: [{ id, title, view_url, ..., dataset?: {...} }] }
    const items = (data.items ?? []).map((t: any) => ({
      id: t.id,
      title: t.title,
      view_url: t.view_url ?? null,
      thumbnail_url: t.thumbnail?.url ?? null,
